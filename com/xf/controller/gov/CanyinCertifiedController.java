...
@Scope("prototype")
@Controller
@RequestMapping(value = "/canyin/certified")
public class CanyinCertifiedController {

	@Autowired
	private CanyinCertifiedService theService;
	@Autowired
	private LoginManage loginManage;
	@Autowired
	private ResultObj result;

	private CanyinCertified check_input(HttpServletRequest request) {
		CanyinCertified ret = new CanyinCertified();
		...
		ret.setStorename(request.getParameter("storename"));
		ret.setStoreaddr(request.getParameter("storeaddr"));
		ret.setLegalPerson(request.getParameter("legalPerson"));
		ret.setLegalPersonID(request.getParameter("legalPersonID"));
		ret.setContact(request.getParameter("contact"));
		ret.setContactNo(request.getParameter("contactNo"));
		ret.setCertifiedID(request.getParameter("certifiedID"));
		ret.setCertifiedType(request.getParameter("certifiedType"));
		ret.setQlevel(request.getParameter("qlevel"));
		ret.setCertifiedGov(request.getParameter("certifiedGov"));
		
		s = request.getParameter("staffNum");
		if (s != null && Tools.isInteger(s)) ret.setStaffNum(Integer.parseInt(s));
		s = request.getParameter("incoming");
		if (s != null && Tools.isNumeric(s)) ret.setIncoming(Double.parseDouble(s));
		s = request.getParameter("tax");
		if (s != null && Tools.isNumeric(s)) ret.setTax(Double.parseDouble(s));
	...
	public Object getCheck(HttpServletRequest request) {
		try {
			...
			CanyinCertified input = check_input(request);
			if (input.getStorename() == null)
				return result.setStatus(-3, "no data.");

			input.setAccountid(companyid);
			CanyinCertified search = theService.getByField(input);
	...
	public Object getById(@PathVariable int id, HttpServletRequest request) {
		try {
			...
			CanyinCertified search = theService.getById(id);
			if (null == search)
				return result.setStatus(-3, "No data.");
	...
	public Object getByYear(@PathVariable int year, HttpServletRequest request) {
		try {
			...
			CanyinCertified input = new CanyinCertified();
			input.setFillyear(year);
			input.setAccountid(companyid);
			List<CanyinCertified> list = theService.getByYear(input);
			if (list != null)
