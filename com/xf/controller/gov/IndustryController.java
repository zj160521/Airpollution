...
public class IndustryController {
	...
	@Autowired
	private IndustryService theService;
	...
	private Industry check_input(HttpServletRequest request) {
		...
		ret.setHouseNumber(request.getParameter("houseNumber"));
		ret.setIndustryCode(request.getParameter("industryCode"));
		ret.setOpenStatus(request.getParameter("openStatus"));
		...
		s = request.getParameter("coalConsumption");
		if (s != null && Tools.isNumeric(s)) ret.setCoalConsumption(Double.parseDouble(s));
	...
	public Object getCheck(HttpServletRequest request) {
		try {
			...
			Industry input = check_input(request);
			...
			Industry search = theService.getByField(input);
	...
	public Object getById(@PathVariable int id, HttpServletRequest request) {
		try {
			...
			Industry search = theService.getById(id);
			if (null == search)
				return result.setStatus(-3, "No data.");
	...
	public Object getByYear(@PathVariable int year, HttpServletRequest request) {
		try {
			...
			Industry input = new Industry();
			input.setFillyear(year);
			input.setAccountid(companyid);
			List<Industry> list = theService.getByYear(input);
			if (list != null)
