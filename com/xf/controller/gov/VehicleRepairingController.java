...
@Scope("prototype")
@Controller
@RequestMapping(value = "/vehicle/repairing")
public class VehicleRepairingController extends BaseController {

	@Autowired
	private VehicleRepairingService theService;
	@Autowired
	private LoginManage loginManage;
	@Autowired
	private ResultObj result;

	private VehicleRepairing check_input(HttpServletRequest request) {
		VehicleRepairing ret = new VehicleRepairing();
		...
		s = request.getParameter("class1Number");
		if (s != null && Tools.isInteger(s)) ret.setClass1Number(Integer.parseInt(s));
		s = request.getParameter("class2Number");
		if (s != null && Tools.isInteger(s)) ret.setClass2Number(Integer.parseInt(s));
		s = request.getParameter("class3Number");
		if (s != null && Tools.isInteger(s)) ret.setClass3Number(Integer.parseInt(s));
	...
	public Object getCheck(HttpServletRequest request) {
		try {
			...
			VehicleRepairing input = check_input(request);
			if (input.getTown() <= 0)
				return result.setStatus(-3, "no data.");
			
			input.setAccountid(companyid);
			List<VehicleRepairing> search = theService.getByTown(input);
	...
	public Object getById(@PathVariable int id, HttpServletRequest request) {
		try {
			...
			VehicleRepairing search = theService.getById(id);
			if (null == search)
				return result.setStatus(-3, "No data.");
	...
	public Object getByYear(@PathVariable int year, HttpServletRequest request) {
		try {
			...
			VehicleRepairing input = new VehicleRepairing();
			input.setFillyear(year);
			input.setAccountid(companyid);
			List<VehicleRepairing> list = theService.getByYear(input);
			if (list != null&&list.size()>0) {
				result.put("data", list);
				if (list.get(0).getStatus() == 3) result.put("Status", 3);
				if (list.get(0).getStatus() == 1) result.put("Status", 1);
			}else{
				result.put("data", list);
			}
